...
### 2026-09-15: v3.4 30 天音感挑战

- 新增每天约 8 分钟的30天主训练线，将课程、练习、挑战、奖励和周报告串成闭环。
- 内容覆盖单音、音程、麦克风模唱、和弦与旋律，不再只是散落功能入口。
- 增加60%挑战门槛、自然日解锁和收藏奖励，形成次日回访理由。
- 第一版进度保存在本地浏览器，未实现跨设备同步。

### 2026-09-15: v3.3.1 小规模试用前收尾

- 新增 PWA 更新提示，避免已打开页面长期停留旧版本。
- 修复音高 RAF、录音 URL、课程与哼唱延迟任务的卸载清理。
- 高风险 Hook warning 清零，总 warning 从 21 降至 10。
- 增加公开课程配置快照、自动校验和恢复手册；用户数据不进入 Git。
- 已发送测试账号密码恢复邮件，需在邮箱内完成重置。
- 目标阶段切换为 5–10 人小规模试用，暂停继续增加功能。

### 2026-09-15: v3.2 稳定性重启

- 增加测试、CI、错误边界、路由懒加载和可回滚发布记录。
- 修复认证重复监听、游客数据同步时序、麦克风权限悬挂、迟到媒体流泄漏和游戏次数重复累计。
- 优化音高检测计算频率、首屏音频请求、PWA 写入策略和生产拆包。
- 后端不可用时保留游客听音与音准练习入口。
- 生产依赖漏洞归零；17 项测试、生产构建、GitHub Actions 和 Vercel Production deployment 通过。
- 当前环境无法连接 Supabase，登录、云端进度和好友 PK 留待具备后端网络条件时补充全链路验收。
- 后续确认 Supabase 项目是免费套餐闲置暂停；已从控制台恢复。Auth、技能、课程、登录会话、XP 和课程进度读取验证正常。

### 2026-02-04: Bug 修复 + 踩坑总结

#### 修复的 Bug

**1. 新手引导页面完成后反复循环**
- 用户完成 onboarding 后会被反复重定向回 onboarding 页面
- **原因**: `Onboarding.tsx` 只更新了 localStorage，没有调用 `setOnboardingCompleted()` 更新 Zustand 全局状态
- **修复**: 在 `handleComplete` 和 `handleSkip` 中调用 `setOnboardingCompleted()`

**2. 游客登录引导弹窗自动消失**
- 弹窗出现约1秒后自动消失
- **原因**: 
  - z-index 不够高（50），可能被其他元素遮挡
  - 结果页面按钮没有禁用，用户可能误触导致页面导航
- **修复**: 
  - z-index 从 50 提高到 100
  - 弹窗显示时禁用"下一关"、"返回目录"、"再练一次"按钮
  - 遮罩层添加 `onClick stopPropagation`

**3. GuestOnboardingCheck 状态管理错误**
- 使用 `useState` 存储重定向状态，一旦设为 true 就不会重置
- **修复**: 改用直接条件判断，同时检查全局状态和 localStorage

#### 踩坑记录

**1. 状态更新不完整 (重复犯错)**
- ❌ 问题：只更新 localStorage，不更新全局状态（类似之前"分数保存逻辑"问题）
- ✅ 教训：**任何需要持久化的状态，必须同时更新 localStorage 和 Zustand store**

**2. 组件重定向使用 useState (新问题)**
- ❌ 问题：用 `useState` + `useEffect` 控制 `<Navigate />`，状态不会自动重置
- ✅ 解决：直接在 render 中判断条件返回 `<Navigate />`，不要用中间状态

**3. 弹窗 z-index 太低 (新问题)**
- ❌ 问题：z-index: 50 可能被页面其他元素（如动画、浮动按钮）遮挡
- ✅ 解决：重要弹窗（登录、提示）使用 z-index: 100+

**4. 弹窗显示时未禁用背后交互 (新问题)**
- ❌ 问题：弹窗背后的按钮仍可点击，导致意外导航
- ✅ 解决：弹窗显示时，禁用背后所有交互元素
...
| 错误类型 | 之前案例 | 今天案例 |
|---------|---------|---------|
| 状态同步不完整 | 分数保存只更新数据库不更新 UI | onboarding 只更新 localStorage 不更新 store |
| 边界情况处理 | `.single()` 查询空记录报错 | `useState` 重定向状态不重置 |
| UI 层级问题 | - | 弹窗 z-index 太低 |

#### 经验总结

1. **双重状态同步**: localStorage ⟷ Zustand store 必须同步更新
2. **条件渲染优于状态驱动**: 对于导航/重定向，直接判断比 useState 更可靠
3. **弹窗设计原则**: 高 z-index + 禁用背后交互 + 明确关闭方式
4. **测试完整流程**: 不只测试正常路径，也要测试从头到尾的完整用户旅程

---

### 2026-02-04: 第二轮 Bug 修复

#### 修复的 Bug

**1. 设置界面保存按钮持续加载**
- **原因**: `supabase.from('profiles').upsert()` 缺少 `onConflict: 'id'` 参数
- **历史踩坑**: 项目日志明确记录过 "upsert 需要指定 onConflict"，但仍然犯错
- **修复**: 添加 `{ onConflict: 'id' }`

**2. 用户无法退出登录**
- **原因**: signOut 函数没有完善的错误处理，API 失败时状态未清除
- **修复**: 添加 try-catch，确保无论 API 是否成功都清除本地状态

**3. 游客注册后数据未同步**
- **原因**: `syncGuestDataToCloud` 只同步了 leaderboard，没有同步 XP
- **修复**: 扩展函数，同步游客完成的课程数转换为初始 XP

#### 踩坑记录

**1. upsert 缺少 onConflict (重复犯错！)**
- ❌ 这是第二次犯同样的错误
- ❌ 明明在日志中记录过，但新代码仍然遗漏
- ✅ 教训：**每次写 upsert 时，必须检查是否有 onConflict**

**2. 异步函数错误处理不完整**
- ❌ signOut 中 API 调用失败时，后续状态清除不执行
- ✅ 教训：**异步函数必须用 try-catch 包裹，确保后续逻辑执行**

**3. 功能实现不完整**
- ❌ syncGuestDataToCloud 只实现了部分同步逻辑
- ✅ 教训：**同步功能需要列清单：哪些数据需要同步？检查是否都覆盖了**

#### 错误模式分析

| 错误 | 首次出现 | 今日重犯 | 根因 |
|-----|---------|---------|------|
| upsert 缺 onConflict | 2026-01-23 | ✅ 重犯 | 复制代码时未检查 |
| 状态同步不完整 | 今天早些时候 | ✅ 同类 | 只考虑正常路径 |

**改进措施**:
1. 写 upsert 时强制检查 onConflict
2. 异步函数统一使用 try-catch
3. 同步功能写前先列数据清单
