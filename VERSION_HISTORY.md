...
**新增功能**:
- 🏆 **联赛系统**: 每周与全服玩家同台竞技
- 📊 **6个联赛等级**: 青铜 → 白银 → 黄金 → 钻石 → 大师 → 传奇
- 👥 **分组匹配**: 每组30人，按周XP排名
- ⬆️ **晋级机制**: 组内前20%晋级，后20%降级
- 🎁 **联赛奖励**: 每周结算XP奖励 + XP加成倍率
...
**新增文件**:
- `src/pages/League.tsx` - 联赛页面
- `src/utils/leagueService.ts` - 联赛服务
- `src/lib/add-league-system.sql` - 数据库架构（需手动执行）

**修改文件**:
- `src/App.tsx` - 添加联赛路由
- `src/pages/Compete.tsx` - 联赛入口
